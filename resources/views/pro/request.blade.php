...
<section class="py-20 px-4 min-h-screen">
    <div class="max-w-2xl mx-auto">
        ...
        <form method="POST" action="{{ route('pro.request.submit') }}" class="space-y-6">
            ...
            <div>
                <label for="email" class="block text-sm font-heading uppercase tracking-wider text-gray-400 mb-2">Email Google *</label>
                <input type="email" name="email" id="email" value="{{ old('email') }}" required placeholder="votre.email@gmail.com"
                    class="w-full bg-mw-dark border border-white/10 rounded-lg px-4 py-3 text-white placeholder-gray-600 focus:border-mw-red focus:ring-1 focus:ring-mw-red transition-colors">
                <p class="text-gray-600 text-xs mt-1">La connexion à l'Espace Pro se fait exclusivement via Google.</p>
        ...
        <div class="text-center mt-12 pt-8 border-t border-white/5">
            ...
            <a href="http://localhost/auth/google" class="inline-flex items-center gap-2 px-6 py-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-sm font-heading uppercase tracking-wider transition-colors">
